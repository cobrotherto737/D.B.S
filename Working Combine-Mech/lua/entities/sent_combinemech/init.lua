...
function ENT:PhysicsUpdate( physics )
	...
	if !self.mech or self.mech == NULL or self.mech == nil then return false end
	...
	if self.MechHealth > 0 then
		if (!IsValid(self.UserSeat)) then return end
		...
		if self.User != NULL && IsValid(self.User) then
			...
			elseif (not(self.User:KeyDown( IN_ATTACK )) or self.wepType != 6) && self.GravProbeSpawned == true then
...
function ENT:Think()
	...
	if !self.mech or self.mech == NULL or self.mech == nil then
		self:Remove()
	end
...
function ENT:FixPropProtection(ply)
	...
	self.mech:SetNetworkedEntity("ASS_Owner", ply)
